...
let clientCache: KaminoVaultClient | null = null;

function getClient(): KaminoVaultClient {
  if (!clientCache) {
    clientCache = new KaminoVaultClient(
      getKaminoRpc(),
      RECENT_SLOT_DURATION_MS,
...
async function fetchVaultList(): Promise<CachedVaultData[]> {
  if (Date.now() - vaultCacheTs < CACHE_TTL_MS && vaultCache.length > 0) {
    return vaultCache;
  }

  verbose('Fetching Kamino vault list from API...');
...
async function loadKaminoVault(vaultAddress: string): Promise<KaminoVault> {
...
export class KaminoEarnProvider implements EarnProvider {
  ...
  async getVaults(tokens?: string[]): Promise<EarnVault[]> {
    ...
    if (tokens && tokens.length > 0) {
      ...
      for (const t of tokens) {
        const meta = await resolveToken(t);
    ...
    for (const v of filtered) {
      ...
      try {
        const kVault = await loadKaminoVault(v.address);
        ...
        try {
          ...
          verbose(`Could not compute APY for vault ${v.name || v.address}`);
        }
        ...
        verbose(`Skipping vault ${v.name || v.address}: ${err.message}`);
      }
    }
  ...
  async getPositions(walletAddress: string): Promise<EarnPosition[]> {
    verbose(`Fetching Kamino earn positions for ${walletAddress}`);
    ...
    const results = await Promise.allSettled(
      vaultList.map(async (v) => {
        ...
        const symbol = (await resolveToken(v.tokenMint))?.symbol ?? '';
  ...
  async deposit(walletName: string, token: string, amount: number, vaultId?: string): Promise<EarnWriteResult> {
    ...
    if (vaultId) {
      ...
      verbose(`Auto-selected vault "${best.vaultName}" (APY: ${(best.apy * 100).toFixed(2)}%)`);
    }

    const kVault = await loadKaminoVault(targetVaultAddress);
    const state = await kVault.getState();
    const reservesMap = await client.loadVaultReserves(state as any);
  ...
  async withdraw(walletName: string, token: string, amount: number): Promise<EarnWriteResult> {
    ...
    const kVault = await loadKaminoVault(pos.vaultId);
    const state = await kVault.getState();
    const reservesMap = await client.loadVaultReserves(state as any);
    ...
    verbose(`Withdrawing ${shareAmount.toFixed(0)} shares from vault ${pos.vaultName}`);
    ...
    for (let i = 0; i < ixGroups.length; i++) {
      ...
      if (i > 0) verbose(`Sending withdraw transaction ${i + 1}/${ixGroups.length}...`);
      result = await buildAndSendTransaction(instructions, signer, {
